...
instr1Img = visual.ImageStim(
    win=win,
    name='instr1Img', 
    image='1back.png', mask=None,
    ori=0, pos=(0.5, -0.3), size=(1, 1),
...
instr2 = visual.TextStim(win=win, name='instr2',
    text='This is the end of 1-back trials. You are about to start 2-back trials. This means that instead of pressing ‘space’ whenever the square appears in the same position as on the position on one trial before, you are required to press ‘space’ whenever the square appears in the same position as on the position two trials before. For example if the square appeared in left down corner on trial 1, you should press ‘space’ if the square appears in the left down corner on trial 3. Press ‘s’ to continue.',
    font='Arial',
    pos=(-0.3, 0), height=0.05, wrapWidth=None, ori=0, 
    color='white', colorSpace='rgb', opacity=1, 
    languageStyle='LTR',
...
instr2Image = visual.ImageStim(
    win=win,
    name='instr2Image', 
    image='2back.png', mask=None,
    ori=0, pos=(0.5, -0.15), size=(1, 1),
    color=[1,1,1], colorSpace='rgb', opacity=1,
    flipHoriz=False, flipVert=False,
...
instr3 = visual.TextStim(win=win, name='instr3',
    text='This is the end of 2-back trials. You are about to start 3-back trials. This means that instead of pressing ‘space’ whenever the square appears in the same position as on the position on two trials before, you are required to press ‘space’ whenever the square appears in the same position as on the position three trials before. For example if the square appeared in left down corner on trial 1, you should press ‘space’ if the square appears in the left down corner on trial 4. Press ‘s’ to continue.',
    font='Arial',
    pos=(-0.3, 0), height=0.05, wrapWidth=None, ori=0, 
    color='white', colorSpace='rgb', opacity=1, 
    languageStyle='LTR',
...
instr3Image = visual.ImageStim(
    win=win,
    name='instr3Image', 
    image='3back.png', mask=None,
...
instructions1Components = [instr1, keyResp1, instr1Img]
...
while continueRoutine:
    ...
    if keyResp1.status == STARTED and not waitOnFlip:
        theseKeys = keyResp1.getKeys(keyList=['s'], waitRelease=False)
    ...
    # *instr1Img* updates
    if instr1Img.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        instr1Img.frameNStart = frameN  # exact frame index
        instr1Img.tStart = t  # local t and not account for scr refresh
        instr1Img.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(instr1Img, 'tStartRefresh')  # time at next scr refresh
        instr1Img.setAutoDraw(True)
...
thisExp.addData('instr1Img.started', instr1Img.tStartRefresh)
thisExp.addData('instr1Img.stopped', instr1Img.tStopRefresh)
...
_keyResp2_allKeys = []
# keep track of which components have finished
instructions2Components = [instr2, keyResp2, instr2Image]
...
while continueRoutine:
    ...
    if keyResp2.status == STARTED and not waitOnFlip:
        theseKeys = keyResp2.getKeys(keyList=['s'], waitRelease=False)
    ...
    # *instr2Image* updates
    if instr2Image.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        instr2Image.frameNStart = frameN  # exact frame index
        instr2Image.tStart = t  # local t and not account for scr refresh
        instr2Image.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(instr2Image, 'tStartRefresh')  # time at next scr refresh
        instr2Image.setAutoDraw(True)
...
thisExp.addData('instr2Image.started', instr2Image.tStartRefresh)
thisExp.addData('instr2Image.stopped', instr2Image.tStopRefresh)
...
_keyResp3_allKeys = []
# keep track of which components have finished
instructions3Components = [instr3, keyResp3, instr3Image]
...
while continueRoutine:
    ...
    if keyResp3.status == STARTED and not waitOnFlip:
        theseKeys = keyResp3.getKeys(keyList=['s'], waitRelease=False)
    ...
    # *instr3Image* updates
    if instr3Image.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        instr3Image.frameNStart = frameN  # exact frame index
        instr3Image.tStart = t  # local t and not account for scr refresh
        instr3Image.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(instr3Image, 'tStartRefresh')  # time at next scr refresh
        instr3Image.setAutoDraw(True)
...
thisExp.addData('instr3Image.started', instr3Image.tStartRefresh)
thisExp.addData('instr3Image.stopped', instr3Image.tStopRefresh)
...
for thisTrials3 in trials3:
    ...
    for thisComponent in trialComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    trials3.addData('trialGrid.started', trialGrid.tStartRefresh)
    trials3.addData('trialGrid.stopped', trialGrid.tStopRefresh)
    trials3.addData('trialSquare.started', trialSquare.tStartRefresh)
    trials3.addData('trialSquare.stopped', trialSquare.tStopRefresh)
